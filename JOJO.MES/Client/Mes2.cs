...
namespace JOJO.Mes.Client
{
    ...
    public class Mes2 : IMesSend
    {
        ...
        private Dictionary<string, Mes2Rec> _recDic = new Dictionary<string, Mes2Rec>();
        private bool NowBoardIsCheck { get; set; } = false;
        private Queue<string> SendStrQueue = new Queue<string>();
        private Dictionary<string, Mes2Rec> RecDic
        {
            get => _recDic;
            set
            {
        ...
        private void CheckBoardAccept(MesProcessData data)
        {
            return;
            bool Commond = false;
            data = data.ModifyValueByPath(new string[] { Header, MESSAGENAME }, "NG_EJECT_R");
        ...
        public async Task<MesProcessData> ProcessStop(MesEnum.MachineState on)
        {
            ...
            StationInfo.Add(new MesProcessData { MesName = Station, MesValue = MesApp.Instance.JOJOMesConfig.Mes2.StatueName });
        ...
        public async Task<MesProcessData> Result(MesDynamic data)
        {
            ...
            ProcessData1.Add(new MesProcessData { MesName = value, MesValue = MesApp.Instance.JOJOMesConfig.Mes2.StatueName.ToString() });
        ...
        private async Task<bool> AwaitReceive(MesProcessData Data)
        {
            ...
            try
            {
                ...
                Mes2Rec Mes2Rec = new Mes2Rec();
                string Time = DateTime.Now.ToString();
                Mes2Rec.Cts = cts1;
                Mes2Rec.Data = Data;
                Mes2Rec.ReSend++;
                string ReceiveMESSAGEID = Data.FindValueByPath(new string[] { Heade, MESSAGEID }).ToString();
                if (RecDic.ContainsKey(ReceiveMESSAGEID))
                {
                    RecDic[ReceiveMESSAGEID] = Mes2Rec;
                }
                else
                {
                    RecDic.Add(ReceiveMESSAGEID, Mes2Rec);
                }
                await Task.Run(async () =>
                {
                    while (true)
                    {
                        ...
                        catch (OperationCanceledException)
                        {
                            ...
                            {
                                RecDic.Clear();
                                MesLog.Error("Mes返回信息3次失败。接口为：" + Data.FindValueByPath(new string[] { Heade, MESSAGENAME }).ToString() + " 时间为：" + Time +
                                            "随机ID为：" + ReceiveMESSAGEID);
                                MesModel.SetMachinAlarm("Mes返回信息3次失败");
                            }
                            return;
                        }
                        if (RecDic[ReceiveMESSAGEID].ReSend++ >= MesApp.Instance.JOJOMesConfig.Mes2.ReNumber)
                        {
                            RecDic.Clear();
        ...
        private void CheckSend(MesProcessData data)
        {
            ...
            if (RecDic.ContainsKey(ReceiveMESSAGEID))
            {
                ...
                {
                    RecDic[ReceiveMESSAGEID].Result = true;
                    RecDic[ReceiveMESSAGEID].Cts.Cancel();
                }
            }
            else
        ...
        public void MesSend_Accept(MesProcessData Data)
        {
            ...
            {
                MesModel.SetMachineLog("Mes2 没有连接");
        ...
        {
            List<MesProcessData> Returndata = new List<MesProcessData>();
            Returndata.Add(MesModel.SetResultOKEnable());
            return MesModel.ReturnData(Returndata);
        ...
        private void HeartTimeAndIsConnect()
        {
            Task task1 = Task.Run(() =>
            {
                while (true)
                {
                    if (!AwaitHeartTime(MesApp.Instance.JOJOMesConfig.Mes2.HeartTime))
                    {
        ...
        public async Task<MesProcessData> MesLogin(MesDynamic dynamic)
        {
            ...
            List<MesProcessData> Returndatas = new List<MesProcessData>();
            Returndatas.Add(MesModel.SetResultOKEnable(await MesSend(datas, dynamic), false));
            return MesModel.ReturnData(Returndatas);
        }
